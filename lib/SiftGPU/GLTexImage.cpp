...
void GLTexImage::InitTexture(int width, int height, int clamp_to_edge) {
  ...
  glTexImage2D(_texTarget, 0, _iTexFormat, _texWidth, _texHeight, 0, GL_RGBA,
			   GL_FLOAT, NULL);
...
void GLTexImage::InitTexture(int width, int height, int clamp_to_edge,
  ...
  glTexImage2D(_texTarget, 0, format, _texWidth, _texHeight, 0, GL_RGBA,
			   GL_FLOAT, NULL);

  UnbindTex();
}
void GLTexImage::BindTex() { glBindTexture(_texTarget, _texID); }

void GLTexImage::UnbindTex() { glBindTexture(_texTarget, 0); }
...
void GLTexImage::ZeroHistoMargin() { ZeroHistoMargin(_imgWidth, _imgHeight); }
...
void GLTexImage::DrawImage() { DrawQuad(); }
...
void GLTexImage::AttachToFBO(int i) {
  glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, i + GL_COLOR_ATTACHMENT0_EXT,
							_texTarget, _texID, 0);
}
...
void GLTexImage::DrawQuadDS(int scale) { DrawScaledQuad(float(scale)); }

void GLTexImage::DrawQuadUS(int scale) { DrawScaledQuad(1.0f / scale); }
  ...
  switch (gl_format) {
	...
	case GL_RGBA:
	  ...
	  for (i = 0; i < hs; i++, line += linestep) {
		for (j = 0, p = line; j < ws; j++, p += step) {
		  //*po++ = int(p[0]*0.299 + p[1] * 0.587 + p[2]* 0.114 + 0.5);
		  *po++ = ((19595 * p[0] + 38470 * p[1] + 7471 * p[2]) /
				   (65535.0f * factor));
		}
	  }
	  break;
	...
	case GL_BGRA:
	  ...
	  for (i = 0; i < hs; i++, line += linestep) {
		for (j = 0, p = line; j < ws; j++, p += step) {
		  *po++ = ((7471 * p[0] + 38470 * p[1] + 19595 * p[2]) /
				   (65535.0f * factor));
		}
	  }
	  break;
...
int GLTexInput::DownSamplePixelDataF(unsigned int gl_format, int width,
									 int height, int ds, const float* pin,
									 float* pout, int skip) {
  int step, linestep;
  int i, j;
  int ws = width / ds - skip;
...
int GLTexInput::SetImageData(int width, int height, const void* data,
  ...
  int simple_format = IsSimpleGlFormat(
	  gl_format, gl_type);  // no cpu code to handle other formats
  int ws, hs, done = 1;
  ...
  if (simple_format &&
	  (width > _texMaxDim || height > _texMaxDim ||
	   GlobalUtil::_PreProcessOnCPU) &&
  ...
  if (GlobalUtil::_UseCUDA || GlobalUtil::_UseOpenCL) {
	...
	if (!simple_format) {
	  ...
	  return 0;
	} else if (_down_sampled > 0 || gl_format != GL_LUMINANCE ||
			   gl_type != GL_FLOAT) {
	  ...
	  if (gl_type == GL_UNSIGNED_BYTE)
		DownSamplePixelDataI2F(gl_format, width, height, 1 << _down_sampled,
							   ((const unsigned char*)data), _converted_data,
							   skip);
	  else if (gl_type == GL_UNSIGNED_SHORT)
		DownSamplePixelDataI2F(gl_format, width, height, 1 << _down_sampled,
							   ((const unsigned short*)data), _converted_data,
							   skip);
	  else
		DownSamplePixelDataF(gl_format, width, height, 1 << _down_sampled,
	  ...
	  if (skip > 0) {
		for (int i = 1; i < _imgHeight; ++i) {
		  float *dst = ((float*)data) + i * tWidth,
				*src = ((float*)data) + i * _imgWidth;
	...
	if (simple_format &&
		(_down_sampled > 0 ||
		 (gl_format != GL_LUMINANCE && GlobalUtil::_PreProcessOnCPU))) {
	  if (gl_type == GL_UNSIGNED_BYTE) {
		unsigned char* newdata = new unsigned char[_imgWidth * _imgHeight];
		DownSamplePixelDataI(gl_format, width, height, 1 << _down_sampled,
							 ((const unsigned char*)data), newdata);
		glTexImage2D(
			_texTarget, 0, GL_LUMINANCE32F_ARB,  // internal format changed
			_imgWidth, _imgHeight, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, newdata);
		delete[] newdata;
	  } else if (gl_type == GL_UNSIGNED_SHORT) {
		...
		glTexImage2D(
			_texTarget, 0, GL_LUMINANCE32F_ARB,  // internal format changed
			_imgWidth, _imgHeight, 0, GL_LUMINANCE, GL_UNSIGNED_SHORT, newdata);
		delete[] newdata;
	  } else if (gl_type == GL_FLOAT) {
		float* newdata = new float[_imgWidth * _imgHeight];
		DownSamplePixelDataF(gl_format, width, height, 1 << _down_sampled,
							 (float*)data, newdata);
		glTexImage2D(_texTarget, 0,
					 GL_LUMINANCE32F_ARB,  // internal format changed
					 _imgWidth, _imgHeight, 0, GL_LUMINANCE, GL_FLOAT, newdata);
		delete[] newdata;
	  } else {
		// impossible
		done = 0;
	  ...
	  if (gl_format == GL_LUMINANCE || gl_format == GL_LUMINANCE_ALPHA) {
		...
		glTexImage2D(_texTarget, 0, GL_LUMINANCE32F_ARB, _imgWidth, _imgHeight,
		...
		if (ShaderMan::HaveShaderMan())
		  ...
		  _rgb_converted =
			  0;  // In CUDA mode, the conversion will be done by CUDA kernel
	  }
	}
	UnbindTex();
  }
...
int GLTexInput::LoadImageFile(char* imagepath, int& w, int& h) {
  ...
  if (fscanf(file, "%s %d %d %d", buf, &width, &height, &cn) < 4 || cn > 255 ||
	  width < 0 || height < 0) {
	fclose(file);
  ...
  if (strcmp(buf, "P5") == 0) {
	...
	for (j = 0; j < num; j++) {
	  fread(buf, 1, 3, file);
	  *pixels++ =
		  int(0.10454f * buf[2] + 0.60581f * buf[1] + 0.28965f * buf[0]);
	}
...
int GLTexImage::CopyToPBO(GLuint pbo, int width, int height, GLenum format) {
  ...
  FrameBufferObject fbo;
  GLint bsize,
	  esize = width * height * sizeof(float) * (format == GL_RGBA ? 4 : 1);
  AttachToFBO(0);
...
void GLTexInput::VerifyTexture() {
  ...
  glTexImage2D(_texTarget, 0, GL_LUMINANCE32F_ARB,  // internal format changed
			   _imgWidth, _imgHeight, 0, GL_LUMINANCE, GL_FLOAT, _pixel_data);
  UnbindTex();
  _data_modified = 0;
}
...
void GLTexImage::CopyFromPBO(GLuint pbo, int width, int height, GLenum format) {
  ...
  if (width > 0 && height > 0) {
	...
	glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, width, height, format,
					GL_FLOAT, 0);
